...
export function encrypt(plainText, key) {
  ...
  const preEncoded = encrypted.replace(/\+/g, '%2B').replace(/\//g, '%2F');

  if (process.env.NODE_ENV !== 'production') {
    console.log('[ICICI][encrypt] rawBase64:', encrypted);
    console.log('[ICICI][encrypt] encoded:', preEncoded);
    console.log('[ICICI][encrypt] hasUnencodedPlusOrSlash:', /[+/]/.test(preEncoded));
  }
...
export function decrypt(encryptedText, key) {
  const normalizedCipherText = String(encryptedText)
    .replace(/%2B/g, '+')
    .replace(/%2F/g, '/');

  const decipher = crypto.createDecipheriv(ALGO, Buffer.from(key, 'utf8'), null);
  let decrypted = decipher.update(normalizedCipherText, 'base64', 'utf8');
...
export function buildPaymentUrl({ amount, name, mobile, email, sevaType }) {
  ...
  const query = [
    ['merchantid', encodeURIComponent(String(merchantId))],
