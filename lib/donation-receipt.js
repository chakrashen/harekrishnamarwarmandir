...
export async function createDonationReceipt({ donation, txnId, refNo }) {
  ...
  const receiptUrl = response.data?.message?.url || response.data?.message?.receipt?.url || null;
  const receiptId = response.data?.message?.receipt?.receipt_id || response.data?.message?.receipt?.name || null;
