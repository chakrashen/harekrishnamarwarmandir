...
const DEFAULT_PREACHER = 'DMT';
const DEFAULT_COMPANY = 'Hare Krishna Movement Jaipur';
const DEFAULT_PAYMENT_METHOD = 'Gateway';
...
function normalizeReceiptRemarks(value, fallback = '') {
...
function buildReceiptPayload({ donation, txnId, refNo }) {
  ...
  const remarks = normalizeReceiptRemarks(txnId, refNo || donation?.ref_no);
  ...
  const paymentMethod = process.env.DONATION_PAYMENT_METHOD || DEFAULT_PAYMENT_METHOD;
  const configuredSevaType = normalizeOptionalText(process.env.DONATION_RECEIPT_SEVA_TYPE);
  const configuredSevaSubtype = normalizeOptionalText(process.env.DONATION_RECEIPT_SEVA_SUBTYPE);
  const tryPatronTagging = process.env.DONATION_TRY_PATRON_TAGGING === 'true';
  const sevaType = configuredSevaType || normalizeOptionalText(receiptMeta.seva_type) || donation?.seva_type;
  const sevaSubtype = configuredSevaSubtype
    || normalizeOptionalText(receiptMeta.seva_subtype)
    || (configuredSevaType ? normalizeOptionalText(donation?.seva_type) : '');

  // Include original seva selection in remarks if it was overridden/generalized
  let finalRemarks = remarks;
  if (donation?.seva_type && donation?.seva_type !== sevaSubtype) {
    finalRemarks = `${donation.seva_type} | ${remarks}`.substring(0, 140);
  }
  ...
  if (!sevaType) missing.push('seva_type');
  ...
  const payload = {
    donation: {
      ...
      try_patron_tagging: tryPatronTagging,
  ...
  if (sevaSubtype) payload.donation.seva_subtype = sevaSubtype;
...
export async function createDonationReceipt({ donation, txnId, refNo }) {
  ...
  const normalizedTxnId = normalizeReceiptRemarks(txnId);
  const payloadResult = buildReceiptPayload({ donation, txnId: normalizedTxnId, refNo });
  if (!payloadResult.ok) {
  ...
  const masked = {
    ...
    txnSuffix: normalizedTxnId ? String(normalizedTxnId).slice(-6) : '',
